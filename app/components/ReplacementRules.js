...
import { applyReplacements, paintReplacements } from '../replacements.js';
...
export default {
  ...
  setup(props) {
    ...
    const painted = computed(() => paintReplacements(props.prompt || ''));
    ...
    return {
      ...
      painted, ruleColor, ruleLive, dotTitle,
    };
  },
  template: `
        ...
        <div class="rmx-repl-cols">
          <div class="rmx-repl-list">
            ...
            <div v-for="(r,i) in replacements" :key="i" class="rmx-repl-row">
              <span class="rmx-repl-dot" :class="{off: !ruleLive(r)}" :style="{ background: ruleColor(i) }"
                    :title="dotTitle(r)"></span>
              <input type="checkbox" v-model="r.on" @change="saveReplacements" title="Enable this rule">
              <span class="rmx-repl-find">
                <input type="text" class="rmx-inp" placeholder="find, or [keyword]" v-model="r.from"
                       @focus="openMenu(i)" @click="openMenu(i)" @blur="closeMenu" @keydown.esc="onFindEsc"
                ...
                <div v-if="menuFor === i && (menuList.found.length || menuList.other.length)" class="rmx-kwmenu" @mousedown.prevent>
                  ...
                  <button v-for="k in menuList.found" :key="'f'+k.token" type="button" class="rmx-kw"
                          :title="'Replace ' + k.token + ' wherever it appears in the prompt'" @click="chooseKeyword(r, k.token)">
                  ...
                  <button v-for="k in menuList.other" :key="'o'+k.token" type="button" class="rmx-kw off"
                          :title="k.token + ' is not in this prompt — a rule for it changes nothing until it is'" @click="chooseKeyword(r, k.token)">
                    <span class="rmx-kw-tok">{{ k.token }}</span>
                    <span v-if="ruleFor(k.token)" class="rmx-kw-used">has a rule</span>
                  </button>
                </div>
